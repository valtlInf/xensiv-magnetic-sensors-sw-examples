                       ...
                       uint32 trans_cnt, TDMA_Transfer_Size datawidth, TDMA_Increment_Mode increment)
{
  ...
  if (((uint32_t)increment & DMA_SRC_INCREMENT) == DMA_SRC_INCREMENT)
  {
    entry->Src_End_Ptr = addr_src + (trans_cnt * ((uint32)1u << ((uint8)datawidth))) - 1;
    entry->Control.bit.Src_Inc = (uint8)datawidth;
  }
  else
  {
    entry->Src_End_Ptr = addr_src;
    entry->Control.bit.Src_Inc = 3u;
  }

  if (((uint32_t)increment & DMA_DEST_INCREMENT) == DMA_DEST_INCREMENT)
  {
    entry->Dst_End_Ptr = addr_dst + (trans_cnt * ((uint32)1u << ((uint8)datawidth))) - 1;
    entry->Control.bit.Dst_Inc = (uint8)datawidth;
  }
  else
  {
    entry->Dst_End_Ptr = addr_dst;
    entry->Control.bit.Dst_Inc = 3u;
  }
