 ...
 * CALIB_DATA_t calibration_store_params;
 *
 * float64_t differential_uncalibrated_angle_read = 0;
 * uint32_t sinP_in_LSB = 0;
 * uint32_t cosP_in_LSB = 0;
 * uint32_t sinN_in_LSB = 0;
 * uint32_t cosN_in_LSB = 0;
...
static float64_t ONECALIB_ONECALIBDiffCalculate(const CALIB_DATA_t *calib_param);
 ...
 * Nothing
 */
void ONECALIB_InitSensorData(float64_t sinP_in_LSB, float64_t cosP_in_LSB, float64_t sinN_in_LSB, float64_t cosN_in_LSB) {
...
static void ONECALIB_AngleUncalibDiffCalculate(void) {
    ...
    //sensor_data.uncalibratedAngle = atan2((float64_t) sensor_data.diff_Y, (float64_t) sensor_data.diff_X) + ANGLE180_RAD;
 ...
 */
static float64_t ONECALIB_ONECALIBDiffCalculate(const CALIB_DATA_t *calib_param) {
    float64_t corr_X = ((float64_t) sensor_data.diff_X - calib_param->mean_offsetX) / calib_param->mean_amplitudeX;
    float64_t corr_Y = ((float64_t) sensor_data.diff_Y - calib_param->mean_offsetY) / calib_param->mean_amplitudeY;
...
static void ONECALIB_CalibOneDirectionMinMax(ROTATION_DATA_STORE_t* rotation_data_store) {
    ...
    //if(((float64_t)ANGLE45_H) <= ((float64_t)sensor_data.uncalibratedAngle))
...
bool SME1_angleComparison_OneTimeCompensation(const CALIB_DATA_t *calib_param )
{
    ...
    volatile float64_t CosP_display_calibrated = ( (float64_t)sensor_data.cosP_LSB - calib_param->rotation_data_CW.offsetXP) / calib_param->rotation_data_CW.amplitudeXP;
    volatile float64_t SinP_display_calibrated = ( (float64_t)sensor_data.sinP_LSB - calib_param->rotation_data_CW.offsetYP) / calib_param->rotation_data_CW.amplitudeYP;
    volatile float64_t CosN_display_calibrated = ( (float64_t)sensor_data.cosN_LSB - calib_param->rotation_data_CW.offsetXN) / calib_param->rotation_data_CW.amplitudeXN;
    volatile float64_t SinN_display_calibrated = ( (float64_t)sensor_data.sinN_LSB - calib_param->rotation_data_CW.offsetYN) / calib_param->rotation_data_CW.amplitudeYN;
...
bool SME2_1_vectorLengthCheck_OneTimeCompensation(const CALIB_DATA_t *calib_param)
{
    ...
    volatile float64_t V_P = sqrt( pow(CosP_ortho_corrected,2) + pow(SinP_ortho_corrected,2)  );
    volatile float64_t V_N = sqrt( pow(CosN_ortho_corrected,2) + pow(SinN_ortho_corrected,2)  );
...
bool SME2_3_vectorLengthCheck_OneTimeCompensation(const CALIB_DATA_t *calib_param)
{
    ...
    volatile float64_t V_PD = sqrt( pow((SinP_ortho_corrected),2) + pow((CosP_ortho_corrected),2) );
