...
float64_t ONGO_ONGODiffCalculate(const ONGO_CALIB_DATA_t *calib_param);
...
void ONGO_InitCalibData(ONGO_CALIB_DATA_t *calib_param, int32_t max_diff_sin_LSB, int32_t max_diff_cos_LSB, int32_t min_diff_sin_LSB,
        int32_t min_diff_cos_LSB, float64_t sin_45, float64_t cos_45, float64_t sin_135, float64_t cos_135) {
...
void ONGO_InitCalibData2(ONGO_CALIB_DATA_t *calib_param, int32_t max_SinP_LSB, int32_t max_CosP_LSB, int32_t max_SinN_LSB,
        int32_t max_CosN_LSB, int32_t min_SinP_LSB, int32_t min_CosP_LSB, int32_t min_SinN_LSB, int32_t min_CosN_LSB, float64_t sin_45,
        float64_t cos_45, float64_t sin_135, float64_t cos_135) {

    calib_param->rotation_data.angle135found = false;
    calib_param->rotation_data.angle45found = false;
    calib_param->rotation_data.nr_valid_rotations = 0u;

    calib_param->rotation_data.maxCosN = max_CosP_LSB - min_CosN_LSB;
    calib_param->rotation_data.minCosN = min_CosP_LSB - max_CosN_LSB;
    calib_param->rotation_data.maxSinN = max_SinP_LSB - min_SinN_LSB;
    calib_param->rotation_data.minSinN = min_SinP_LSB - max_SinN_LSB;

    calib_param->rotation_data.maxCosP = max_CosP_LSB - min_CosN_LSB;
    calib_param->rotation_data.minCosP = min_CosP_LSB - max_CosN_LSB;
    calib_param->rotation_data.maxSinP = max_SinP_LSB - min_SinN_LSB;
    calib_param->rotation_data.minSinP = min_SinP_LSB - max_SinN_LSB;
...
void ONGO_CalculateDiff() {
    ...
    sensor_data.uncalibratedAngle = atan2((float64_t) sensor_data.diff_Y, (float64_t) sensor_data.diff_X);
}
 ...
 */
float64_t ONGO_ONGODiffCalculate(const ONGO_CALIB_DATA_t *calib_param) {
    float64_t corr_X = ((float64_t)sensor_data.diff_X - calib_param->rotation_data.offsetX) / calib_param->rotation_data.amplitudeX;
    float64_t corr_Y = ((float64_t)sensor_data.diff_Y - calib_param->rotation_data.offsetY) / calib_param->rotation_data.amplitudeY;

    float64_t ortho_Y = (corr_Y - (corr_X * calib_param->sin_ortho)) / calib_param->cos_ortho;
...
void ONGO_CalibOneDirectionMinMax(ONGO_ROTATION_DATA_STORE_t* rotation_data_store) {
    ...
    {
        rotation_data_store->X45 = (float64_t) sensor_data.diff_X;
        rotation_data_store->Y45 = (float64_t) sensor_data.diff_Y;
    ...
    {
        rotation_data_store->X135 = (float64_t) sensor_data.diff_X;
        rotation_data_store->Y135 = (float64_t) sensor_data.diff_Y;
 ...
 */
void ONGO_CalibCalculateOneDirection(ONGO_ROTATION_DATA_STORE_t *rotation_data_store) {
    rotation_data_store->amplitudeXN = ((float64_t) rotation_data_store->maxCosN - (float64_t) rotation_data_store->minCosN) / 2.00;
    rotation_data_store->amplitudeYN = ((float64_t) rotation_data_store->maxSinN - (float64_t) rotation_data_store->minSinN) / 2.00;
    rotation_data_store->amplitudeXP = ((float64_t) rotation_data_store->maxCosP - (float64_t) rotation_data_store->minCosP) / 2.00;
    rotation_data_store->amplitudeYP = ((float64_t) rotation_data_store->maxSinP - (float64_t) rotation_data_store->minSinP) / 2.00;
    ...
    rotation_data_store->offsetXN = ((float64_t) rotation_data_store->maxCosN + (float64_t) rotation_data_store->minCosN) / 2.00;
    rotation_data_store->offsetYN = ((float64_t) rotation_data_store->maxSinN + (float64_t) rotation_data_store->minSinN) / 2.00;
    rotation_data_store->offsetXP = ((float64_t) rotation_data_store->maxCosP + (float64_t) rotation_data_store->minCosP) / 2.00;
    rotation_data_store->offsetYP = ((float64_t) rotation_data_store->maxSinP + (float64_t) rotation_data_store->minSinP) / 2.00;
...
{
    volatile float64_t CosP_display_calibrated = ( (float64_t)sensor_data.cosP_LSB - calib_param->rotation_data.offsetXP) / calib_param->rotation_data.amplitudeXP;
    volatile float64_t SinP_display_calibrated = ( (float64_t)sensor_data.sinP_LSB - calib_param->rotation_data.offsetYP) / calib_param->rotation_data.amplitudeYP;
    volatile float64_t CosN_display_calibrated = ( (float64_t)sensor_data.cosN_LSB - calib_param->rotation_data.offsetXN) / calib_param->rotation_data.amplitudeXN;
    volatile float64_t SinN_display_calibrated = ( (float64_t)sensor_data.sinN_LSB - calib_param->rotation_data.offsetYN) / calib_param->rotation_data.amplitudeYN;


    volatile float64_t SinP_ortho_corrected = (SinP_display_calibrated - (CosP_display_calibrated * calib_param->sin_ortho)) / calib_param->cos_ortho;
    volatile float64_t SinN_ortho_corrected = (SinN_display_calibrated - (CosN_display_calibrated * calib_param->sin_ortho)) / calib_param->cos_ortho;

    volatile float64_t angle_calibrated_P = atan2l(SinP_ortho_corrected, CosP_display_calibrated) * RAD2DEGFACTOR;
    volatile float64_t angle_calibrated_N = atan2l(SinN_ortho_corrected, CosN_display_calibrated) * RAD2DEGFACTOR;
...
bool SME2_1_vectorLength_AutoCalibration(const ONGO_CALIB_DATA_t *calib_param)
{
    ...
    volatile float64_t SinP_ortho_corrected = (float64_t)( (SinP_display_calibrated - (CosP_display_calibrated * calib_param->sin_ortho)) / calib_param->cos_ortho);
    volatile float64_t CosP_ortho_corrected = (float64_t)( (CosP_display_calibrated - (SinP_display_calibrated * calib_param->sin_ortho)) / calib_param->cos_ortho);
    volatile float64_t SinN_ortho_corrected = (float64_t)( (SinN_display_calibrated - (CosN_display_calibrated * calib_param->sin_ortho)) / calib_param->cos_ortho);
    volatile float64_t CosN_ortho_corrected = (float64_t)( (CosN_display_calibrated - (SinN_display_calibrated * calib_param->sin_ortho)) / calib_param->cos_ortho);


    volatile float64_t V_P = (float64_t)sqrt( pow(CosP_ortho_corrected,2) + pow(SinP_ortho_corrected,2)  );
    volatile float64_t V_N = (float64_t)sqrt( pow(CosN_ortho_corrected,2) + pow(SinN_ortho_corrected,2)  );
...
{
    volatile double CosP_display_calibrated = ( (float64_t)sensor_data.cosP_LSB - calib_param->rotation_data.offsetXP) / calib_param->rotation_data.amplitudeXP;
    volatile double SinP_display_calibrated = ( (float64_t)sensor_data.sinP_LSB - calib_param->rotation_data.offsetYP) / calib_param->rotation_data.amplitudeYP;


    volatile double SinP_ortho_corrected = (float64_t)( (SinP_display_calibrated - (CosP_display_calibrated * calib_param->sin_ortho)) / calib_param->cos_ortho);
    volatile double CosP_ortho_corrected = (float64_t)( (CosP_display_calibrated - (SinP_display_calibrated * calib_param->sin_ortho)) / calib_param->cos_ortho);
...
bool SME2_3_vectorLengthCheck_AutoCalibration(const ONGO_CALIB_DATA_t *calib_param)
{
    ...
    float64_t corr_X = ((float64_t)sensor_data.diff_X - calib_param->rotation_data.offsetX) / calib_param->rotation_data.amplitudeX  * 5 /4095;
    float64_t corr_Y = ((float64_t)sensor_data.diff_Y - calib_param->rotation_data.offsetY) / calib_param->rotation_data.amplitudeY  * 5 /4095;

    float64_t ortho_Y = (corr_Y - (corr_X * calib_param->sin_ortho)) / calib_param->cos_ortho;

    volatile float64_t V_PD = sqrt( pow((ortho_Y),2) + pow((corr_X),2) );
...
bool SME3_commonModeCheck(const ONGO_CALIB_DATA_t *calib_param)
{
    ...
    float64_t Vcm_s = (float64_t)SinP_display_calibrated + (float64_t)SinN_display_calibrated;
    float64_t Vcm_c = (float64_t)CosP_display_calibrated + (float64_t)CosN_display_calibrated;
