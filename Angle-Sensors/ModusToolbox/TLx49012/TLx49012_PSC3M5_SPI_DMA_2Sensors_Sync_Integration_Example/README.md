# TLx49012 PSC3M5_CC2 SPI DMA 2-Sensor Sync Integration Example

<br>

## 1. Introduction

This code example provides a starting point for interfacing two TLx49012 angle sensors with a PSOC&trade; Control C3 microcontroller, using **SPI with DMA**. <br>
...
This example code performs continuous **synchronized** readouts of two TLx49012 angle sensors via SPI using DMA, at a frequency of ~20Hz. <br>
...
- **SCB5**, for SPI communication with the sensors (two slave selects, DMA-driven);
- **SCB1**, for UART communication with the PC;
- **DMA**, for SPI TX and RX transfers;
...
1. Under **Trigger Level**:
    - **RX FIFO Level**: 0 (read trigger for every byte);
    - **TX FIFO Level**: 4 (write trigger for 4 byte - 32 bit frame);
1. Under **API Mode**:
    - **API Mode**: High Level;
...
10. Under **Port 8**, make sure that the pins have the appropriate **Drive Mode**:
...
10. Select the **16.5 bit Divider 0** clock (should already be enabled);
...
Two DMA channels are used — one for SPI TX and one for SPI RX. For each channel:

1. Select the **DMA** tab of the **Device Configurator**;
1. Select and enable a **DMA channel** (e.g., **txDma** and **rxDma**, renaming optional);
1. Configure **RX DMA**:![RX_DMA_Config](doc/TLx49012_DMA_RX_Config.png)
4. Configure **TX DMA**:![TX_DMA_Config](doc/TLx49012_DMA_TX_Config.png)
...
- `MCU` folder contains all the microcontroller-specific initialization and peripheral functions:
    ...
    - `SPI` folder contains the SPI initialization sequence and the low-level DMA-based SPI data transfer function;
    - `DMA` folder contains the TX and RX DMA channel configuration and their completion interrupt handlers;
...
- `PSC3M5_SPI_Init()`: Configures and enables the SCB0 SPI block, then initializes the TX and RX **DMA channels** used for SPI data transfers via `configure_tx_dma()` and `configure_rx_dma()`;
...
> Both sensors are reset from VM memory using the `STAT_EN` register, so register contents are maintained. A 900µs delay is applied to wait for SPI to become active again. <br>
...
> The command is transferred via DMA and the sensor response is received in the same SPI frame. <br>
