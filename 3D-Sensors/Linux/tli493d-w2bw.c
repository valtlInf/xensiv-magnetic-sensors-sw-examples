...
int tli493d_write(int handle, unsigned char addr, unsigned char reg, unsigned char value);
...
int main(){
    ...
    result = tli493d_write(i2c_hd, 0x35, 0x11, 0x15);
    if (result == 0){
    ...
    result = tli493d_write(i2c_hd, 0x35, 0x10, 0x20);
    if (result == 0){
        printf("Error in writing \n \r");
        return 0;
    }
